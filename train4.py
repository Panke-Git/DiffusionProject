...
if __name__ == "__main__":
    ...
    if opt['phase'] == 'train':
        best_dir = opt['path'].get(
            'best', os.path.join(opt['path']['experiments_root'], 'best'))
        ...
        def save_best_metrics(tag, info_dict):
            _dump_json(os.path.join(
                best_dir, 'best_{}_metrics.json'.format(tag)), info_dict)
            ...
            summary['best_{}'.format(tag)] = info_dict
        ...
        logger.info('Loaded %d previous validation records from %s.',
                    len(validation_history), val_log_path)
        logger.info('Resume best records: %s', best_records)

        train_loss_sum = 0.0
        train_loss_count = 0
        validation_scheduler = ValidationScheduler(
            opt, n_iter, start_step=current_step,
            ...
            for _, train_data in enumerate(train_loader):
                ...
                if wandb_logger:
                    wandb_logger.log_metrics({
                        'train/batch_size': train_data['target'].shape[0]
                    }, commit=False)
                diffusion.feed_data(train_data)
                diffusion.optimize_parameters()

                cur_logs = diffusion.get_current_log()
                ...
                if validation_jobs:
                    ...
                    for job in validation_jobs:
                        info = run_validation(
                            diffusion, val_loader, opt, current_epoch,
                            current_step, job['tag'], job['cfg'],
                            tb_logger=tb_logger, wandb_logger=wandb_logger)
                        info['train_loss'] = avg_train_loss
                        if should_track_best(opt, job['tag']):
                            best_records = update_best_records(
                                diffusion, best_records, info,
                                save_best_metrics)
                            save_best_records(best_dir, best_records)
                        if should_save_checkpoint(opt, job['tag']):
                            logger.info(
                                'Saving checkpoint after %s validation.',
                                job['tag'])
                            diffusion.save_network(current_epoch, current_step)
                            checkpoint_saved_this_step = True
                        if wandb_logger:
                            wandb_metrics = {
                                'validation/{}/val_loss'.format(job['tag']):
                                    info['val_loss'],
        ...
        if final_job is not None:
            ...
            if should_track_best(opt, final_job['tag']):
                best_records = update_best_records(
                    diffusion, best_records, info, save_best_metrics)
                save_best_records(best_dir, best_records)
            if should_save_checkpoint(opt, final_job['tag']):
                logger.info('Saving checkpoint after %s validation.',
                            final_job['tag'])
                diffusion.save_network(current_epoch, current_step)

        logger.info('======Training Finished: Best Summary======')
        logger.info('Best loss: {:.6e} @ epoch {} iter {}'.format(
            best_records['loss']['value'], best_records['loss']['epoch'],
            best_records['loss']['iter']))
        logger.info('Best psnr: {:.4f} @ epoch {} iter {}'.format(
            best_records['psnr']['value'], best_records['psnr']['epoch'],
            best_records['psnr']['iter']))
        logger.info('Best ssim: {:.4f} @ epoch {} iter {}'.format(
            best_records['ssim']['value'], best_records['ssim']['epoch'],
            best_records['ssim']['iter']))
        save_best_records(best_dir, best_records)
