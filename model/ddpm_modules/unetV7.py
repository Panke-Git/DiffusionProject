...
from model.DGRM_PLUS import DGRMPlus
...
class UNet(nn.Module):
    def __init__(
        ...
        dgrm_plus_opt = dgrm_plus_opt or {}
        self.use_dgrm_plus = bool(dgrm_plus_opt.get('enabled', True))
        dgrm_apply_mode = dgrm_plus_opt.get('apply_mode', 'stage_last')
        dgrm_apply_resolutions = dgrm_plus_opt.get('apply_resolutions', [32, 64, 128, 256])
        if dgrm_apply_resolutions is None:
            dgrm_apply_resolutions = []
        dgrm_apply_resolutions = {int(res) for res in dgrm_apply_resolutions}
        dgrm_kwargs = {
            'hidden_channels': dgrm_plus_opt.get('hidden_channels', None),
            'alpha_init': float(dgrm_plus_opt.get('alpha_init', 0.01)),
            'use_detail_residual': bool(dgrm_plus_opt.get('use_detail_residual', True)),
            'use_edge_guard': bool(dgrm_plus_opt.get('use_edge_guard', True)),
            'edge_strength': float(dgrm_plus_opt.get('edge_strength', 1.0)),
            'modulation_scale': float(dgrm_plus_opt.get('modulation_scale', 0.5)),
        }

        def should_attach_dgrm(resolution, block_idx, block_count):
            if not self.use_dgrm_plus or int(resolution) not in dgrm_apply_resolutions:
                return False
            ...
            if dgrm_apply_mode in ['none', 'disabled']:
                return False
            raise ValueError(f'Unknown DGRM+ apply_mode: {dgrm_apply_mode}')
        ...
        for ind in reversed(range(num_mults)):
            ...
            for block_idx in range(0, block_count):
                ...
                if should_attach_dgrm(stage_res, block_idx, block_count):
                    self.up_dgrms[str(layer_idx)] = DGRMPlus(
                        channels=pre_channel,
                        **dgrm_kwargs
                    )
            if not is_last:
