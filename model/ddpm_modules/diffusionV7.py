...
from model.depth_guided_adaptive_regularization import DepthGuidedAdaptiveRegularizer
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.depth_regularizer = DepthGuidedAdaptiveRegularizer(
            lambda_tv=reg_lambda_tv,
            lambda_edge=reg_lambda_edge,
            depth_gain=reg_depth_gain,
            edge_suppress_tau=reg_edge_tau,
        )
    ...
    def estimate_depth(self, condition_x):
        condition_x = torch.clamp((condition_x + 1.0) / 2.0, 0.0, 1.0)
        return self.depth_estimator(condition_x).to(dtype=condition_x.dtype)
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None, condition_depth=None):
        if condition_x is not None:
            x_recon = self.predict_start_from_noise(
                x, t=t, noise=self.denoise_fn(torch.cat([condition_x, x], dim=1), t, depth=condition_depth))
        else:
            x_recon = self.predict_start_from_noise(
    ...
    def p_sample(self, x, t, clip_denoised=True, repeat_noise=False, condition_x=None, condition_depth=None):
        ...
        model_mean, _, model_log_variance = self.p_mean_variance(
            x=x, t=t, clip_denoised=clip_denoised, condition_x=condition_x, condition_depth=condition_depth)
    ...
    def p_sample_loop(self, x_in, continous=False):
        ...
        if not self.conditional:
            ...
            img = torch.randn(shape, device=device)
            ret_img = x
            condition_depth = self.estimate_depth(x)
            for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
                img = self.p_sample(img, torch.full(
                    (b,), i, device=device, dtype=torch.long), condition_x=x, condition_depth=condition_depth)
                if i % sample_inter == 0:
                    ret_img = torch.cat([ret_img, img], dim=0)
        if continous:
            return ret_img
    ...
    def p_losses(self, x_in, noise=None):
        x_start = x_in['target']  # GT
        cond_img = x_in['input']  # 已经是 3C 条件图
        ...
        # 1) 用条件图估计伪深度，供 DGRMPlus 和正则项共用
        cond_img_01 = (cond_img + 1.0) / 2.0
        with torch.no_grad():
            depth_map = self.depth_estimator(cond_img_01)

        # 2) 原始 DiffWater 噪声预测 + DGRMPlus 深度调制
        if not self.conditional:
            noise_pred = self.denoise_fn(x_noisy, t)
        else:
            noise_pred = self.denoise_fn(torch.cat([cond_img, x_noisy], dim=1), t, depth=depth_map)

        # 3) 原始 diffusion loss. Keep the diffusion term on the same
        ...
        # 5) 由于正则模块一般按 [0,1] 计算，这里把 [-1,1] 转到 [0,1]
        x0_pred_01 = (x0_pred + 1.0) / 2.0

        # 6) 深度引导的自适应正则
        reg_loss, reg_info = self.depth_regularizer(
            pred_x0=x0_pred_01,
            depth=depth_map,
            guide=cond_img_01
        )
