...
from model.DGRM_A4 import DGRM_A4
...
class UNet(nn.Module):
    def __init__(
        ...
        dgrm_a4_opt = dgrm_a4_opt or {}
        self.use_dgrm_a4 = bool(dgrm_a4_opt.get('enabled', True))
        dgrm_apply_mode = dgrm_a4_opt.get('apply_mode', 'stage_last')
        dgrm_apply_resolutions = dgrm_a4_opt.get('apply_resolutions', [64])
        if dgrm_apply_resolutions is None:
            dgrm_apply_resolutions = []
        dgrm_apply_resolutions = {int(res) for res in dgrm_apply_resolutions}
        dgrm_kwargs = {
            'cond_channels': int(dgrm_a4_opt.get('cond_channels', 32)),
            'gaussian_kernel': int(dgrm_a4_opt.get('gaussian_kernel', 5)),
            'gaussian_sigma': float(dgrm_a4_opt.get('gaussian_sigma', 1.0)),
            'gamma_scale_low': float(dgrm_a4_opt.get('gamma_scale_low', 0.1)),
            'beta_scale_low': float(dgrm_a4_opt.get('beta_scale_low', 0.05)),
            'gamma_scale_high': float(dgrm_a4_opt.get('gamma_scale_high', 0.1)),
            'use_depth_edge': bool(dgrm_a4_opt.get('use_depth_edge', True)),
            'alpha_init': float(dgrm_a4_opt.get('alpha_init', 1.0)),
        }

        def should_attach_dgrm(resolution, block_idx, block_count):
            if not self.use_dgrm_a4 or int(resolution) not in dgrm_apply_resolutions:
                return False
            if dgrm_apply_mode == 'all_resblocks':
                return True
            if dgrm_apply_mode == 'stage_first':
                return block_idx == 0
            if dgrm_apply_mode == 'stage_last':
                return block_idx == block_count - 1
            if dgrm_apply_mode in ['none', 'disabled']:
                return False
            raise ValueError(f'Unknown DGRM_A4 apply_mode: {dgrm_apply_mode}')

        ups = []
        self.up_dgrms = nn.ModuleDict()
        for ind in reversed(range(num_mults)):
            is_last = (ind < 1)
            use_attn = (now_res in attn_res)
            channel_mult = inner_channel * channel_mults[ind]
            stage_res = now_res
            block_count = res_blocks + 1
            for block_idx in range(0, block_count):
                ...
                layer_idx = len(ups) - 1
                pre_channel = channel_mult
                if should_attach_dgrm(stage_res, block_idx, block_count):
                    self.up_dgrms[str(layer_idx)] = DGRM_A4(
                        channels=pre_channel,
                        **dgrm_kwargs
                    )
            if not is_last:
    ...
    def forward(self, x, time, depth=None):
        ...
        x = self.mid_uas(x, t)
        for layer_idx, layer in enumerate(self.ups):
            if isinstance(layer, ResnetBlocWithAttn):
                x = layer(torch.cat((x, feats.pop()), dim=1), t)
                dgrm_key = str(layer_idx)
                if dgrm_key in self.up_dgrms:
                    x = self.up_dgrms[dgrm_key](x, depth)
            else:
                x = layer(x)

        return self.final_conv(x)

    def reset_dgrm_a4_parameters(self):
        for module in self.up_dgrms.values():
