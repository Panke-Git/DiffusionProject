...
from .MIPTVDepthEstimatorV2 import Block1_MIPTVV2
...
class UIEDataset(Dataset):
    def __init__(self, dataroot, resolution=256, split='train', data_len=-1):
        ...
        # Your depth prior module
        self.front_prior = Block1_MIPTVV2(input_channels=3, output_channels=3, n_channels=32, ch_mults=[1, 2, 2, 4],
                                          n_blocks=2)
    ...
    def __getitem__(self, index):
        ...
        # Convert to tensor in [-1, 1], keeping original DiffWater pipeline
        [input, target] = Util.transform_augment([input, target], split=self.split, min_max=(-1, 1))

        # SceneDepthEstimatorADMM expects RGB tensor in [0, 1]
        input_01 = (input + 1.0) / 2.0

        with torch.no_grad():
            J = self.front_prior(input_01.unsqueeze(0)).squeeze(0)  # [1, H, W], [0,1]

        # Normalize depth to [-1, 1] to match DiffWater input range
        # depth = depth * 2.0 - 1.0  # [1, H, W]

        # Scheme 3: RGB(3) + Depth(1) -> 4-channel condition
        # input = torch.cat([input, depth], dim=0)  # [4, H, W]
